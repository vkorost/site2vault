...
description: Complete API reference for the Python Agent SDK, including all functions,
  types, and classes.
---
...
SessionCreates new session each timeReuses same sessionConversationSingle exchangeMultiple exchanges in same contextConnectionManaged automaticallyManual controlStreaming Input✅ Supported✅ SupportedInterrupts❌ Not supported✅ SupportedHooks✅ Supported✅ SupportedCustom Tools✅ Supported✅ SupportedContinue Chat❌ New session each time✅ Maintains conversationUse CaseOne-off tasksContinuous conversations
...
User-set session tag (see [[Agent SDK reference - Python - Claude Code Docs|```
tag_session()
```]])
...
requiredThe session ID to retrieve messages for
...
Returns [[Agent SDK reference - Python - Claude Code Docs|```
SDKSessionInfo
```]], or
...
requiredNew title. Must be non-empty after stripping whitespace
...
Rename the most recent session so it’s easier to find later. The new title appears in [[Agent SDK reference - Python - Claude Code Docs|```
...
Maintains a conversation session across multiple exchanges. This is the Python equivalent of how the TypeScript SDK’s
...
. See [File checkpointing](https://code.claude.com/docs/en/agent-sdk/file-checkpointing)

```
get_mcp_status()
```

Get the status of all configured MCP servers. Returns [[Agent SDK reference - Python - Claude Code Docs|```
McpStatusResponse
```]]
...
Stop a running background task. A [[Agent SDK reference - Python - Claude Code Docs|```
...
) are plain dicts at runtime and require key access:
...
to block tools. See [[Configure permissions - Claude Code Docs-dbd6de#Allow and deny rules|Permissions]]
...
Enable file change tracking for rewinding. See [File checkpointing](https://code.claude.com/docs/en/agent-sdk/file-checkpointing)
...
Beta features to enable. See [[Agent SDK reference - Python - Claude Code Docs|```
SdkBeta
...
). See [[Get structured output from agents - Claude Code Docs|Structured outputs]] for details
...
Environment variables merged on top of the inherited process environment. See [[Environment variables - Claude Code Docs|Environment variables]] for variables the underlying CLI reads
...
Deprecated - File-like object for debug output. Use
...
Tool permission callback function. See [[Agent SDK reference - Python - Claude Code Docs|Permission types]] for details
...
Include partial message streaming events. When enabled, [[Agent SDK reference - Python - Claude Code Docs|```
StreamEvent
...
Load custom plugins from local paths. See [[Plugins in the SDK - Claude Code Docs|Plugins]] for details
...
Configure sandbox behavior programmatically. See [[Agent SDK reference - Python - Claude Code Docs|Sandbox settings]] for details
...
(CLI defaults: all sources)Control which filesystem settings to load. Pass
...
Deprecated - Maximum tokens for thinking blocks. Use
...
[```
SessionStore
```](https://code.claude.com/docs/en/agent-sdk/session-storage#the-session-store-interface)
...
Mirror session transcripts to an external backend so any host can resume them. See [Persist sessions to external storage](https://code.claude.com/docs/en/agent-sdk/session-storage)
...
YesJSON Schema definition for output validation
...
NoAdditional instructions to append to the preset system prompt

```
exclude_dynamic_sections
```

NoMove per-session context such as working directory, git status, and memory paths from the system prompt into the first user message. Improves prompt-cache reuse across users and machines. See [[Modifying system prompts - Claude Code Docs#Improve prompt caching across users and machines|Modify system prompts]]
...
loads the same filesystem settings as the Claude Code CLI: user, project, and local. Managed policy settings are loaded in all cases. See [[Use Claude Code features in the SDK - Claude Code Docs#What settingSources does not control|What settingSources does not control]] for inputs that are read regardless of this option, and how to disable them.
...
NoMCP servers available to this agent. Each entry is a server name or an inline
...
NoPermission mode for tool execution within this agent. See [[Agent SDK reference - Python - Claude Code Docs|```
PermissionMode
```]]
...
, which uses Python snake\_case for the equivalent top-level fields such as
...
is a dataclass, passing a snake\_case keyword raises a
...
The configuration of an MCP server as reported by [[Agent SDK reference - Python - Claude Code Docs#Methods|```
get_mcp_status()
```]]. This is the union of all [[Agent SDK reference - Python - Claude Code Docs|```
McpServerConfig
...
is the serializable form of [[Agent SDK reference - Python - Claude Code Docs|```
McpSdkServerConfig
```]] with only
...
Response from [[Agent SDK reference - Python - Claude Code Docs#Methods|```
...
Status of a connected MCP server, contained in [[Agent SDK reference - Python - Claude Code Docs|```
McpStatusResponse
```]].
...
(optional)Server name and version (
...
(optional)Error message if the server failed to connect
...
[[Agent SDK reference - Python - Claude Code Docs|```
McpServerStatusConfig
```]] (optional)Server configuration. Same shape as [[Agent SDK reference - Python - Claude Code Docs|```
McpServerConfig
...
(optional)Tools provided by this server, each with
...
For complete information on creating and using plugins, see [[Plugins in the SDK - Claude Code Docs|Plugins]].
...
Per-message token usage (same keys as [[Agent SDK reference - Python - Claude Code Docs|```
ResultMessage.usage
```]])
...
dict maps model names to per-model usage. The inner dict keys use camelCase because the value is passed through unmodified from the underlying CLI process, matching the TypeScript [[Agent SDK reference - TypeScript - Claude Code Docs|```
ModelUsage
```]] type:
...
Estimated cost in USD for this model, computed client-side. See [Track cost and usage](https://code.claude.com/docs/en/agent-sdk/cost-tracking) for billing caveats.
...
Rate limit state carried by [[Agent SDK reference - Python - Claude Code Docs|```
RateLimitEvent
```]].
...
Emitted when a background task starts. A background task is anything tracked outside the main turn: a backgrounded Bash command, a [[Agent SDK reference - Python - Claude Code Docs#Monitor|Monitor]] watch, a subagent spawned via the Agent tool, or a remote agent. The
...
For a comprehensive guide on using hooks with examples and common patterns, see the [[Intercept and control agent behavior with hooks - Claude Code Docs|Hooks guide]].
...
- ```
  ...
  (see [[Agent SDK reference - Python - Claude Code Docs|```
  HookInput
  ```]])
- ```
  tool_use_id
...
Returns a [[Agent SDK reference - Python - Claude Code Docs|```
HookJSONOutput
```]] that may contain:
...
(optional)Current permission mode
...
(optional)Subagent identifier, present when the hook fires inside a subagent
...
(optional)Subagent type, present when the hook fires inside a subagent
...
(optional)Whether the failure was caused by an interrupt
...
(optional)Suggested permission updates from the CLI
...
value. For full details on available fields per hook event, see [[Intercept and control agent behavior with hooks - Claude Code Docs#Outputs|Control execution with hooks]].
...
Asks the user clarifying questions during execution. See [[Handle approvals and user input - Claude Code Docs#Handle clarifying questions|Handle approvals and user input]] for usage details.
Input:
...
Runs a background script and delivers each stdout line to Claude as an event so it can react without polling. Monitor follows the same permission rules as Bash. See the [[Tools reference - Claude Code Docs#Monitor tool|Monitor tool reference]] for behavior and provider availability.
Input:
...
in tool input, which falls back to the [[Agent SDK reference - Python - Claude Code Docs#Permissions Fallback for Unsandboxed Commands|permissions system]]
...
Filesystem and network access restrictions are NOT configured via sandbox settings. Instead, they are derived from [[Claude Code settings - Claude Code Docs#Permission settings|permission rules]]:
...
handler validates these requests carefully.If
