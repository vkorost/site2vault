...
For security and isolation, the SDK should run inside a sandboxed container environment. This provides process isolation, resource limits, network control, and ephemeral filesystems. The SDK also supports

[[Agent SDK reference - TypeScript - Claude Code Docs|programmatic sandbox configuration]]for command execution.
...
Several providers specialize in secure container environments for AI code execution: For self-hosted options (Docker, gVisor, Firecracker) and detailed isolation configuration, see
...
- [[Agent SDK reference - TypeScript - Claude Code Docs|TypeScript SDK - Sandbox Settings]]- Configure sandbox programmatically
- [[Work with sessions - Claude Code Docs|Sessions Guide]]- Learn about session management
- [[Configure permissions - Claude Code Docs-dbd6de|Permissions]]- Configure tool permissions
- [Cost Tracking](https://code.claude.com/docs/en/agent-sdk/cost-tracking)- Monitor API usage
- [[Connect to external tools with MCP - Claude Code Docs|MCP Integration]]- Extend with custom tools
