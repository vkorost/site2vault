...
description: Declare version constraints on plugin dependencies so your plugin keeps
  working when an upstream plugin ships a breaking change.
---
...
[[Discover and install prebuilt plugins through marketplaces - Claude Code Docs|Discover and install plugins]]. For the full manifest schema, see the
...
stringPlugin name. Resolves within the same marketplace as the declaring plugin. Required.
...
in. Cross-marketplace dependencies are blocked unless the target marketplace is listed in [[Constrain plugin dependency versions - Claude Code Docs#Depend on a plugin from another marketplace|in the root marketplace’s]]
...
Version constraints resolve against git tags on the marketplace repository. For Claude Code to find a dependency’s available versions, the upstream plugin’s releases must be tagged using a specific naming convention. Tag each release as
...
A declared dependency is not installed, or it is installed but disabled.Run the
...
The version requirements for a dependency cannot be combined. The error message names the cause: no version satisfies all of the ranges, a range is not valid semver syntax, or the combined ranges are too complex to intersect.Uninstall or update one of the conflicting plugins, fix any invalid
...
The installed dependency’s version is outside this plugin’s declared range.Run
...
tag satisfying the range.Check that the upstream has tagged releases using the convention above, or relax your range.
...
- [[Create plugins - Claude Code Docs|Create plugins]]: build plugins with skills, agents, and hooks
- [[Create and distribute a plugin marketplace - Claude Code Docs|Create and distribute a plugin marketplace]]: host plugins for your team
- [[Plugins reference - Claude Code Docs#Plugin manifest schema|Plugins reference]]: the full
  ...
  schema
- [[Plugins reference - Claude Code Docs#Version management|Version management]]: how a plugin’s own version is resolved and used as the cache key
