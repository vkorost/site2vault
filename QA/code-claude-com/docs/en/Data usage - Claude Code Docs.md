...
is set. To control frequency instead of disabling, set

[[Claude Code settings - Claude Code Docs#Available settings|in your settings file to a probability between]]
...
- [[Zero data retention - Claude Code Docs|Zero data retention]]: available for Claude Code on Claude for Enterprise. ZDR is enabled on a per-organization basis; each new organization must have ZDR enabled separately by your account team
- Local caching: Claude Code clients store session transcripts locally in plaintext under
  ...
  . See[[Explore the .claude directory - Claude Code Docs#Application data|application data]]for what’s stored and how to clear it.

[[Use Claude Code on the web - Claude Code Docs#Delete sessions|Delete sessions]]. Learn more about data retention practices in our

[Privacy Center](https://privacy.anthropic.com/). For full details, please review our
...
ProviderEncryption at restAnthropic APIInfrastructure-level disk encryption (AES-256). Enable
...
[[Use Claude Code on the web - Claude Code Docs|Claude Code on the web]], sessions run in Anthropic-managed virtual machines instead of locally. In cloud environments:
...
[[Security - Claude Code Docs#Cloud execution security|Security]].
...
[Statsig security documentation](https://www.statsig.com/trust/security). To opt out of Statsig telemetry, set the
...
[Sentry security documentation](https://sentry.io/security/). To opt out of error logging, set the
...
command, a copy of their full conversation history including code is sent to Anthropic. The data is encrypted in transit via TLS. Optionally, a GitHub issue is created in the public repository. To opt out, set the
...
By default, error reporting, telemetry, and bug reporting are disabled when using Bedrock, Vertex, or Foundry. Session quality surveys and the WebFetch domain safety check are exceptions and run regardless of provider. You can opt out of all non-essential traffic, including surveys, at once by setting
...
ServiceClaude APIVertex APIBedrock APIFoundry APIStatsig (Metrics)Default on.
...
must be 1.Sentry (Errors)Default on.
...
must be 1.Session quality surveysDefault on.
...
to disable.WebFetch domain safety checkDefault on.
...
[[Claude Code settings - Claude Code Docs|settings reference]]).
...
[[Claude Code settings - Claude Code Docs|settings]]. Disabling the check means WebFetch attempts to retrieve any URL without consulting the blocklist, so combine it with

[[Configure permissions - Claude Code Docs#WebFetch|if you need to restrict which domains Claude can reach.]]
