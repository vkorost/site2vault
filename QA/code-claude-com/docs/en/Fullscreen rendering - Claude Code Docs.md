...
description: Enable a smoother, flicker-free rendering mode with mouse support and
  stable memory usage in long conversations.
---

Fullscreen rendering is an opt-in

[[Fullscreen rendering - Claude Code Docs#Research preview|research preview]]and requires Claude Code v2.1.89 or later. Run
...
[[Claude Code settings - Claude Code Docs#Available settings|and relaunches into fullscreen with your conversation intact, so you can switch mid-session without losing context. Run]]
...
```
[[Fullscreen rendering - Claude Code Docs#Use the mouse|
```
...
-click to open a URL[[Fullscreen rendering - Claude Code Docs#Use the mouse|Use the mouse]]

[[Fullscreen rendering - Claude Code Docs#Keep native text selection|turn it off]]while keeping the flicker-free rendering.
...
- Click a URL or file path to open it. File paths in tool output, like the ones printed after an Edit or Write, open in your default application. Plain
...
Jump to the latest message and re-enable auto-followMouse wheelScroll a few lines at a time
...
[[Customize keyboard shortcuts - Claude Code Docs|Scroll actions]]for the full list of action names, including half-page and full-page variants that have no default binding.
...
Fullscreen rendering works inside tmux, with two caveats. Mouse wheel scrolling requires tmux’s mouse mode. If your
...
Fullscreen rendering is a research preview feature. It has been tested on common terminal emulators, but you may encounter rendering issues on less common terminals or unusual configurations. If you encounter a problem, run
...
[claude-code GitHub repo](https://github.com/anthropics/claude-code/issues). Include your terminal emulator name and version. To turn fullscreen rendering off, run

```
/tui default
```

, or unset the environment variable if you enabled it that way.
