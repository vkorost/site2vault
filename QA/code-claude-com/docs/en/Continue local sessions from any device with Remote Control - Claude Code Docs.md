...
description: Continue a local Claude Code session from your phone, tablet, or any
  browser using Remote Control. Works with claude.ai/code and the Claude mobile app.
---
...
[claude.ai/code](https://claude.ai/code)or the Claude app for
...
- Use your full local environment remotely: your filesystem, [[Connect Claude Code to tools via MCP - Claude Code Docs|MCP servers]], tools, and project configuration all stay available, and typing
...
[[Use Claude Code on the web - Claude Code Docs|Claude Code on the web]], which runs on cloud infrastructure, Remote Control sessions run directly on your machine and interact with your local filesystem. The web and mobile interfaces are just a window into that local session.
...
- Scan the QR code shown alongside the session URL to open it directly in the Claude app. With
...
[[Continue local sessions from any device with Remote Control - Claude Code Docs#Start a Remote Control session|server mode]]instead.
...
- If
  ...
  shows No mobile registered, open the Claude app on your phone so it can refresh its push token. The warning clears the next time Remote Control connects.
...
- One remote session per interactive process: outside of server mode, each Claude Code instance supports one remote session at a time. Use [[Continue local sessions from any device with Remote Control - Claude Code Docs#Start a Remote Control session|server mode]]to run multiple concurrent sessions from a single process.
...
- Extended network outage: if your machine is awake but unable to reach the network for more than roughly 10 minutes, the session times out and the process exits. Run
...
- Ultraplan disconnects Remote Control: starting an [[Plan in the cloud with ultraplan - Claude Code Docs|ultraplan]]session disconnects any active Remote Control session because both features occupy the claude.ai/code interface and only one can be connected at a time.
- Some commands are local-only: commands that open an interactive picker in the terminal, such as
...
This error has three distinct causes. Run
...
- Your Team or Enterprise admin hasn’t enabled it: Remote Control is off by default on these plans. An admin can enable it at [claude.ai/admin-settings/claude-code](https://claude.ai/admin-settings/claude-code)by turning on the Remote Control toggle. This is a server-side organization setting, not a[[Configure permissions - Claude Code Docs#Managed-only settings|managed settings]]key.
...
- [[Use Claude Code on the web - Claude Code Docs|Claude Code on the web]]: run sessions in Anthropic-managed cloud environments instead of on your machine
- [[Plan in the cloud with ultraplan - Claude Code Docs|Ultraplan]]: launch a cloud planning session from your terminal and review the plan in your browser
- [[Push events into a running session with channels - Claude Code Docs|Channels]]: forward Telegram, Discord, or iMessage into a session so Claude reacts to messages while you’re away
- [[Use Claude Code Desktop - Claude Code Docs#Sessions from Dispatch|Dispatch]]: message a task from your phone and it can spawn a Desktop session to handle it
- [[Authentication - Claude Code Docs|Authentication]]: set up
...
- [[CLI reference - Claude Code Docs|CLI reference]]: full list of flags and commands including

  ```
  claude remote-control
  ```
- [[Security - Claude Code Docs|Security]]: how Remote Control sessions fit into the Claude Code security model
- [[Data usage - Claude Code Docs|Data usage]]: what data flows through the Anthropic API during local and remote sessions
