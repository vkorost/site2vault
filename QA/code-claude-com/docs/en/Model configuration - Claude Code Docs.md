...
[1 million token context window](https://platform.claude.com/docs/en/build-with-claude/context-windows#1m-token-context-window)for long sessions
...
[[Claude Code settings - Claude Code Docs#Settings files|managed or policy settings]]to restrict which models users can select. When
...
[[Model configuration - Claude Code Docs|based on the user’s subscription tier]]. Even with
...
[[Claude Code on Amazon Bedrock - Claude Code Docs#Use the Mantle endpoint|Bedrock Mantle endpoint]]is enabled, entries in
...
[[Model configuration - Claude Code Docs#Pin models for third-party deployments|Pin models for third-party deployments]]. The setting still restricts the picker to listed entries, so include the standard aliases alongside any Mantle IDs.
...
field in [[Configure server-managed settings - Claude Code Docs|server-managed settings]].
...
[[Model configuration - Claude Code Docs#Extended context|Extended context]]applies to the
...
- ```
  ...
  flag: pass a level name to set it for a single session when launching Claude Code
- Environment variable: set
...
- Skill and subagent frontmatter: set
  ...
  in a[[Extend Claude with skills - Claude Code Docs#Frontmatter reference|skill]]or[[Create custom subagents - Claude Code Docs#Supported frontmatter fields|subagent]]markdown file to override the effort level when that skill or subagent runs
...
[[Environment variables - Claude Code Docs|environment variables]]. The 1M context window uses standard model pricing with no premium for tokens beyond 200K. For plans where extended context is included with your subscription, usage remains covered by your subscription. For plans that access extended context through extra usage, tokens are billed to extra usage. If your account supports 1M context, the option appears in the model picker (
...
- In [[Customize your status line - Claude Code Docs|status line]](if configured)
- In
...
picker without replacing the built-in aliases. This is useful for LLM gateway deployments or testing model IDs that Claude Code does not list by default.
...
[[Claude Code on Microsoft Foundry - Claude Code Docs|Foundry]], pin model versions before rolling out to users. Without pinning, Claude Code uses model aliases (
...
[Models overview](https://platform.claude.com/docs/en/about-claude/models/overview). To upgrade users to a new model version, update these environment variables and redeploy. To enable

[[Model configuration - Claude Code Docs#Extended context|extended context]]for a pinned model, append
...
[[LLM gateway configuration - Claude Code Docs|LLM gateway]]. They have no effect when connecting directly to
...
[[Model configuration - Claude Code Docs#Adjust effort level|effort levels]]and

[[Common workflows - Claude Code Docs#Use extended thinking (thinking mode)|extended thinking]]by matching the model ID against known patterns. Provider-specific IDs such as Bedrock ARNs or custom deployment names often don’t match these patterns, leaving supported features disabled. Set
...
[[Common workflows - Claude Code Docs#Use extended thinking (thinking mode)|Extended thinking]]
...
[[Claude Code settings - Claude Code Docs#Settings files|settings file]]:

[Models overview](https://platform.claude.com/docs/en/about-claude/models/overview). For dated model IDs, include the date suffix exactly as it appears there. Unknown keys are ignored. Overrides replace the built-in model IDs that back each entry in the
...
[prompt caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)to optimize performance and reduce costs. You can disable prompt caching globally or for specific model tiers:
