...
```mermaid
flowchart TD
    Start([Pop URL from frontier]) --> P{Politeness gate}
    P --> F[HTTP fetch]
    F --> R{Status?}
    R -->|429 / 503| Backoff[Adaptive backoff] --> F
    R -->|2xx| CT{Content-type HTML?}
    R -->|4xx permanent| Fail[Mark failed]
    CT -->|no| Skip[Mark skipped]
    CT -->|yes| MR{Meta refresh?}
    MR -->|yes| Enq[Enqueue target same depth] --> Done
    MR -->|no| AB{Anti-bot signal?}
    AB -->|yes| KS[Kill-switch counter]
    AB -->|no| EX[Extract content]
    EX --> BT{Bot trap?}
    BT -->|yes| Fail
    BT -->|no| CV[Convert to MD with placeholders]
    CV --> W[Write note + sidecar]
    W --> LD[Discover links from raw HTML]
    LD --> Done([Mark done])
```
...
sequenceDiagram
    participant Crawler
    participant Note as Note A
    participant Sidecar as link-index/A.json
    participant Rewriter
    participant Map as URL to filename map
    Crawler->>Note: Write [text](S2V_LINK_0)
    Crawler->>Sidecar: Store S2V_LINK_0 to URL B
    Note over Crawler: ... all pages crawled ...
    Rewriter->>Sidecar: Read S2V_LINK_0
    Rewriter->>Map: Look up URL B
    alt URL B is in vault
        Rewriter->>Note: Replace with [[B|text]]
    else External URL
        Rewriter->>Note: Replace with [text](URL B)
    end
```
...
All CLI flags are documented in [CLI Reference](cli-reference.md). The CLI uses named parameters only (no positional args), with `--url` as the only required flag. Output resolves to `path / name`, where `--name` defaults to a slug derived from the seed URL hostname.
